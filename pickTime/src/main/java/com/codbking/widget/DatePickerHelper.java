...
class DatePickerHelper {
    ...
    private void init() {
        Date date = startDate;
        //获取年 月 日 时 分
        YEAR_START = TimeUtil.getYear(date);
        MONTH_START = TimeUtil.getMoth(date);
        DAY_START = TimeUtil.getDay(date);
        WEEK_START = TimeUtil.getWeek(date);
        HOUR_START = TimeUtil.getHour(date);
        MINUTE_START = TimeUtil.getMinute(date);
    }
    ...
    public String getDisplayWeek(int year, int moth, int day) {
        return weeks[TimeUtil.getWeek(year, moth, day) - 1];
    }
