...
class DownloadAppImpl extends DownloadApp implements Downloader.DownloadListener {
    ...
    public void onSuccess(String taskId, String taskUrl, String localPath) {
        ...
        try {
            Util.installApp(this.context, localPath);
        } catch (Throwable throwable) {
            Logger.e("showInstallApk", throwable);
        }
    }
    ...
    private void showInstall(final String filePath) {
        ...
        try {
            WindowUtil.createAlertDialog(this.context, Device.App.getAppName(context), "安装" + title, "确定", ok, "取消", cancel).show();
        } catch (Throwable e) {
            Logger.e("showInstallApk", e);
            try {
                Util.installApp(this.appContext, filePath);
            } catch (Throwable throwable) {
                Logger.e("showInstallApk", throwable);
            }
        }
    ...
    private NetWorkObservable.NetWorkObserver netWorkObserver = new NetWorkObservable.NetWorkObserver() {
        ...
        private void showNetWork() {
            ...
            try {
                netWorkDialog = WindowUtil.createAlertDialog(context, Device.App.getAppName(context), "当前非WIFI状态，是否继续下载？", "继续", ok, "停止", cancel);
                netWorkDialog.show();
            } catch (Throwable e) {
